...
# Calculates the best value zones for the user
class ZoneCalculator
  ...
  def calculate_farest_zones(origin_zones, destination_zones)
    ...
    # This section is based on the assumption that multi-zone stations have to have consecutive zones
    # (ie can't be zone 1 and 3 without zone 2)
  ...
  def origin_and_destination_multizone(origin_zones, destination_zones)
    ...
    elsif destination_zones.max < origin_zones.min
  ...
  # Technically this will always return the multizone station as the origin, and the single zone station as the
  # destination. Currently, this is not a problem as there is no distinction in fare charge between direction of
  # travel. If an additional charge/ rebate were added depending on direction of travel, this refactor would need
  # to be reverted.
  def one_multizone_one_singlezone(station_1_zones, station_2_zone)
    ...
    elsif station_1_zones.max < station_2_zone

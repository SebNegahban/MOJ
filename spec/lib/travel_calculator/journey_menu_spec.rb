...
RSpec.describe JourneyMenu do
  ...
  describe '#get_origin' do
    before do
      allow(subject).to receive(:get_station).and_return(Station.new('test', 1))
    end

    it 'calls for the station to be entered' do
      expect(subject).to receive(:get_station)

      subject.send(:get_origin)
    end
    ...
    it 'clears the origin station if an error is raised when attempting to find the card' do
      ...
      expect { subject.send(:get_origin) }.to raise_error(InputError)
  ...
  describe '#get_destination' do
    before do
      ...
      allow(subject).to receive(:get_station).and_return(Station.new('test_2', 2))
    end
    ...
    # Had to change the call order to 'expect .to have_received' here as the 'get_destination' method sets the
    ...
    context 'exceptions' do
      it 'raises an error if the origin station has not been defined' do
        ...
        expect { subject.send(:get_destination) }.to raise_error(InputError, /you must select an origin before selecting a destination/i)
      end
    end
  end
  ...
  describe '#get_station' do
    ...
    before do
      ...
      allow(StationMap).to receive(:get_station)
    end

    it 'prompts the user to enter the name of the station' do
      expect { subject.send(:get_station) }.to output(/please enter the name of the station: \n\n/i).to_stdout
    end

    it 'clears the screen' do
      expect(subject).to receive(:clear_screen)

      subject.send(:get_station)
    end

    it 'calls the StationMap to find the station' do
      expect(StationMap).to receive(:get_station).with(station_name)
